...
# -----------------------------
# Load forest stats CSV (optional)
# -----------------------------
stats_csv = "data/file.csv"
if os.path.exists(stats_csv):
    stats_df = pd.read_csv(stats_csv)
    stats_df = stats_df.set_index("District", drop=False)
else:
    print("⚠️ Forest stats CSV not found. Proceeding without it.")
    stats_df = pd.DataFrame()

# -----------------------------
# Simulate poaching points inside forests
# -----------------------------
simulated_points = []
...
for idx, row in forests_gdf.iterrows():
    polygon = row.geometry
    forest_area = row.get("Total", 50)  # fallback
    points_per_forest = max(5, int(forest_area * 0.5))  # adjust density
    minx, miny, maxx, maxy = polygon.bounds
    count = 0
    while count < points_per_forest:
        x = np.random.uniform(minx, maxx)
...
# Create GeoDataFrame of poaching points
poaching_gdf = gpd.GeoDataFrame(geometry=simulated_points, crs=forests_gdf.crs)
poaching_path = "results/poaching_points.geojson"
poaching_gdf.to_file(poaching_path, driver="GeoJSON")
print(f"✅ Simulated poaching points saved: {len(poaching_gdf)} points → {poaching_path}")
...
# -----------------------------
# Count poaching points in each grid cell
# -----------------------------
poaching_sindex = poaching_gdf.sindex
...
plt.title("Nilgiris: Poaching Risk Zones", fontsize=16)
plt.xlabel("Longitude")
plt.ylabel("Latitude")

import matplotlib.patches as mpatches
legend_patches = [
    mpatches.Patch(color="lightgrey", label="Protected Forests"),
...
# -----------------------------
# Save Final Risk Grid
# -----------------------------
final_geojson_path = "results/Nilgiris_risk_grid_final.geojson"
grid_gdf.to_file(final_geojson_path, driver="GeoJSON")
print(f"✅ Risk grid saved as GeoJSON: {final_geojson_path}")
